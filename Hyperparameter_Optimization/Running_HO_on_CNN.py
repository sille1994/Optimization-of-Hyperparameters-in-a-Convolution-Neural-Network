...
class Net(nn.Module):
    ...
    def __init__(self, input_channels=1, input_height=60,             input_width=60, num_classes=10, num_zoom=3):
        ...
        self.localization = nn.Sequential(nn.Conv2d(in_channels=input_channels,            out_channels=8, kernel_size=7, padding=3), nn.MaxPool2d(kernel_size=2,            stride=2), nn.ReLU(inplace=True), nn.Conv2d(in_channels=8,            out_channels=10, kernel_size=5, padding=2), nn.MaxPool2d(kernel_size=2,            stride=2), nn.ReLU(inplace=True))

        # Regressor for the 3 * 2 affine matrix that we use
        # to make the bilinear interpolation for the spatial transformer
        self.fc_loc = nn.Sequential(nn.Linear(in_features=10 * input_height//4 * input_width//4,             out_features=32, bias=True), nn.ReLU(inplace=True), nn.Linear(in_features=32,                    out_features=3 * 2, bias=True))
        ...
        self.conv1 = nn.Conv2d(in_channels=input_channels, out_channels=16, kernel_size=5,             padding=2)
    ...
    def stn(self, x_picture):
        ...
        output_size = torch.Size((x_picture.size()[0], x_picture.size()[1],             x_picture.size()[2]//self.num_zoom, x_picture.size()[3]//self.num_zoom))
...
def train_bayesian_optimization(net: torch.nn.Module, input_picture: DATA,        label_picture: DATA, parameters: Dict[str, float],) -> nn.Module:
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer,                        step_size=int(parameters.get("step_size", 20)),                        gamma=parameters.get("gamma", 1.0),)
...
def eval_bayesian_optimization(net: torch.nn.Module, input_picture: DATA,                                label_picture: DATA, ) -> float:
...
while N < NUM_EPOCHS:
    ...
    try:
        ...
        TRAIN_COST, TRAIN_ACC = train_epoch(input_picture=DATA['x_train'],                                            label_picture=DATA['y_train'],)
